...
const headerIconButtonClass =
  "flex h-8 w-8 items-center justify-center rounded-lg text-[var(--chat-text-soft)] transition-colors duration-200 hover:bg-[var(--chat-interactive-hover)] hover:text-[var(--chat-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--chat-accent)]/25";
...
const ActionViewComp: ReactorType.FC<ActionViewProps> = forwardRef((props, ref) => {
  ...
  return (
    <motion.div
      className={classNames("flex h-full w-full flex-col bg-[var(--chat-surface)]", className)}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{
        duration: 0.4,
