...
- Math (LaTeX): ``$inline$``, ``$$display$$``, AMS environments
  ...
  :mod:`reactor_tool.docgen.mathtext` in every output format.
- Footnotes: ``[^1]`` references + ``[^1]: definition`` (collected into a
  trailing :class:`~reactor_tool.docgen.model.FootnotesBlock`).
...
- ``chart`` / ``metrics`` / ``checklist`` fenced code blocks whose body is
  JSON (or YAML) matching :class:`~reactor_tool.docgen.model.ChartBlock` /
  :class:`~reactor_tool.docgen.model.MetricsBlock` /
  :class:`~reactor_tool.docgen.model.ChecklistBlock` payloads.
...
def parse_markdown_document(markdown_text: str) -> tuple[dict[str, Any], list[Block]]:
    ...
    theme, toc, …) onto the :class:`~reactor_tool.docgen.model.DocumentSpec`.
    """
    if not markdown_text or not markdown_text.strip():
        return {}, []
    tokens = _parser().parse(markdown_text)
...
class _TokenWalker:
    ...
    def _paragraph_from_inline(self, inline: Any) -> Block | None:
        ...
        if len(non_ws) == 1 and non_ws[0].type == "image":
            ...
            alt = img.content or ""
            from reactor_tool.docgen.images import extract_file_id
