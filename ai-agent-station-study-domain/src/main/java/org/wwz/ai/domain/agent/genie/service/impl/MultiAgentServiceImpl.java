...
import org.wwz.ai.domain.agent.genie.agent.enums.AutoBotsResultStatus;
import org.wwz.ai.domain.agent.genie.agent.enums.ResponseTypeEnum;
import org.wwz.ai.domain.agent.genie.config.GenieConfig;
import org.wwz.ai.domain.agent.genie.handler.AgentResponseHandler;
...
public class MultiAgentServiceImpl implements IMultiAgentService {
    ...
    public void handleMultiAgentRequest(AgentRequest autoReq, SseEmitter sseEmitter) {
        ...
        OkHttpClient client = new OkHttpClient.Builder()
                ...
                .readTimeout(genieConfig.getSseClientReadTimeout(), TimeUnit.SECONDS)  // 读取超时：从配置获取（SSE长连接需设置较长时间）
                .writeTimeout(1800, TimeUnit.SECONDS)        // 写入超时：30分钟（适配大请求体的写入场景）
                .callTimeout(genieConfig.getSseClientConnectTimeout(), TimeUnit.SECONDS) // 调用总超时：从配置获取（整个请求的生命周期超时）
                .build();
