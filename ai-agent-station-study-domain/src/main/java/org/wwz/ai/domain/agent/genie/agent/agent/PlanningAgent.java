...
import org.wwz.ai.domain.agent.genie.agent.prompt.PlanningPrompt;
import org.wwz.ai.domain.agent.genie.agent.tool.BaseTool;
import org.wwz.ai.domain.agent.genie.agent.tool.common.PlanningTool;
...
public class PlanningAgent extends ReActAgent {
    ...
    public PlanningAgent(AgentContext context) {
        ...
        // 2. 获取Spring上下文及配置（GenieConfig是业务自定义的配置类，包含大模型、提示词等配置）
        ...
        setMaxSteps(genieConfig.getPlannerMaxSteps()); // 设置最大执行步骤：防止无限循环
        setLlm(new LLM(genieConfig.getPlannerModelName(), "")); // 初始化大模型实例（指定模型名称）

        // 9. 关联上下文&配置计划更新开关
        setContext(context); // 绑定智能体上下文
        setIsColseUpdate("1".equals(genieConfig.getPlanningCloseUpdate())); // 从配置读取是否关闭计划更新（1=关闭）
